    ...
    public function dashboard($id_satker, $thn_dipa)
    {
        ...
        $builder = $this->db->table("({$unionQuery->getCompiledSelect()}) AS ks")
            ->select('ks.id_komponen, c.nama_komponen, ks.id_unsur, u.nama_unsur, 
              ...
              i.bobot_indikator, COALESCE(p_nilai_pm.nilai, 0) AS nilai_pm, COALESCE(p_nilai_pk.nilai, 0) AS nilai_pk', false) // Menangani NULL dengan COALESCE
            ...
            ->join('nilai_pk AS n_pk', 'ks.id_indikator = n_pk.id_pm_satker AND n_pk.id_satker = ' . $this->db->escape($id_satker) . ' AND n_pk.thn_dipa = ' . $this->db->escape($thn_dipa), 'left', false)
            ->join('pilih_nilai AS p_nilai_pm', 'n_pm.nilai = p_nilai_pm.id', 'left')
            ->join('pilih_nilai AS p_nilai_pk', 'n_pk.nilai = p_nilai_pk.id', 'left')
