...
<div class="row">
    <div class="demo-vertical-spacing mb-4"> <!-- progress bar-->
            ...
            <div class="d-flex align-items-center row"> <!-- Tetap menggunakan align-items-center -->
                <div class="col-sm-12"> <!-- Ukuran kolom tetap sama -->
                    <div class="card-body p-2"> <!-- Mengurangi padding untuk tampilan yang lebih kompak -->
                        <h5 class="card-title text-primary mt-1">A. KOMPONEN PENGUNGKIT</h5> <!-- Mengurangi margin atas pada judul -->
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-lg-12 order-0 mt-2">
        ...
        $userRole = $role; // Ambil role user dari session
        ...
        foreach ($syncedData as $index => $row) :
            if (!in_array($row['nama_unsur'], $displayedUnsur)) :
                ...
                // Cek apakah id_unsur dari $syncedData ada di $presen
                foreach ($presen as $p) {
                    if ($p['id_unsur'] == $row['id_unsur']) {
                        // Pilih presentase berdasarkan role user
                        if (in_array($userRole, [3, 4])) { // Role Satker
                            $presentase = $p['presentase_pm'];
                        } elseif (in_array($userRole, [10, 11])) { // Role PK
                            $presentase = $p['presentase_pk'];
                        } elseif (in_array($userRole, [12, 13])) { // Role LVL
                            $presentase = $p['presentase_lvl'];
                        } else {
                            $presentase = 0; // Jika role tidak sesuai
                        }

                        // Atur warna tombol berdasarkan presentase
                        if ($presentase == 100) {
                            $btnClass = "btn-info"; // Hijau jika 100%
                        } else {
                            $btnClass = "btn-dark"; // Default warna jika 0
                        }
        ...
        ?>
                ...
                <button class="btn <?= $btnClass; ?> mb-2 unsur-btn" data-id="<?= $row['id_unsur']; ?>" onclick="handleUnsurClick('<?= $row['id_unsur']; ?>')">
                    <?= $row['nama_unsur']; ?>
                </button>
        <?php
                    ...
                    <div id="accordion<?= $index; ?>"
                        ...
                        <div class="accordion-body">
                            ...
                            <a href="javascript:void(0);" class="btn btn-sm btn-primary btn-info mt-2"
                                onclick="<?= ($role == 4) ? "handleSave($index, '{$indikator['id_indikator']}')" : 'return false;'; ?>" <?= ($role != 4 || !empty($s_pm)) ? 'hidden' : ''; ?>>
                                Simpan
                            </a>
                        </div>
                        ...
                        <div class="accordion-body" <?= (!in_array($role, [10, 11, 12, 13])) ? 'hidden' : ''; ?>>
                            ...
                            <!-- Tombol Simpan -->
                            <a href="javascript:void(0);" class="btn btn-sm btn-primary btn-info mt-2" onclick="<?= ($role == 11) ? "handleSavePK($index, '{$indikator['id_indikator']}')" : 'return false;'; ?>" <?= !($role == 11) ? 'hidden' : ''; ?>>Simpan</a>
                        </div>
...
<div class="d-flex justify-content-between mt-3">
    ...
    <a href="javascript:void(0);" class="btn btn-primary ms-auto" onclick="handleNextClick()">Selanjutnya</a>
</div>
</div>
</div>
</div>
...
<script>
    ...
    function updateProgress(satkerId, tahun) {
        $.ajax({
            url: '<?= base_url('/progress1') ?>', // Endpoint ambil progress terbaru
            type: 'GET',
            data: {
                satkerId: satkerId,
                tahun: tahun
            ...
            <div id="accordion${index}"
                ...
                <div class="accordion-body">
                    ...
                    <a href="javascript:void(0);" 
                        class="btn btn-sm btn-primary btn-info mt-2" 
                        data-index="${index}" 
                        data-id="${row.id_indikator}"
                        ${(role != 4 || s_pm.length > 0) ? 'hidden' : ''}>
                        Simpan
                    </a>
                </div>   
                ...
                <div class="accordion-body" style="${role !== 11 && role !== 10 ? 'display: none;' : ''}">
                    ...
                    <a href="javascript:void(0);" 
                        ...
                        data-index="${index}" 
                        data-id="${row.id_indikator}"
                        onclick="handleSavePK('${index}', '${row.id_indikator}')"
                        ${!(role == 11 ) ? 'hidden' : ''}>
                        Simpan
                    </a>
                </div> 
                ...
                <!-- Bagian Ketiga -->
                <div class="accordion-body" style="${role !== 12 && role !== 13 ? 'display: none;' : ''}">
    ...
    function handleNextClick() {
        window.location.href = "<?= base_url('/soal_hasil?thn=' . $tahun . '&id_satker=' . $id_satker) ?>"; // Ganti dengan URL tujuan
    }
</script>
